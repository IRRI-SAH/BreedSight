...
def compute_genomic_features(X, sample_ids=None, ref_features=None, 
    ...
    if not is_train:
        ...
        # Deep copy and check immutability
        ref_features_copy = copy.deepcopy(ref_features)
        ref_features = copy.deepcopy(ref_features)  # Ensure original is not modified
    ...
    if is_train:
        ...
        # Verify ref_features immutability
        if ref_features != ref_features_copy:
            raise ValueError("ref_features was modified during validation/test processing")
...
def KFoldCrossValidation(training_data, training_additive, val_data=None, val_additive=None,
    ...
    # Check for missing values in input DataFrames
    if training_data.isna().any().any() or training_additive.isna().any().any():
    ...
    if val_data is not None or val_additive is not None:
        ...
        if val_data.isna().any().any() or val_additive.isna().any().any():
    ...
    for outer_fold, (outer_train_index, outer_val_index) in enumerate(outer_kf.split(training_additive_raw), 1):
        ...
        # Log fold details
        logger.info(f"Fold {outer_fold}: Training samples={len(outer_train_index)}, Validation samples={len(outer_val_index)}")
        ...
        if feature_selection:
            ...
            # Log feature counts
            logger.info(f"Fold {outer_fold}: Training features={X_train_final.shape[1]}, Validation features={X_val_final.shape[1]}")
        ...
        print(f"Fold {outer_fold} target scaler mean: {target_scaler.mean_[0]:.4f}, scale: {target_scaler.scale_[0]:.4f}")
        
        # Shuffle training data
        X_train_final, outer_trainy_scaled = shuffle(X_train_final, outer_trainy_scaled, random_state=RANDOM_STATE + outer_fold)
    ...
    if val_data is not None and val_additive is not None:
        ...
        # Feature selection with fresh selector
        if feature_selection:
            if verbose >= 2:
        ...
        # Print scaler parameters for final model
        print(f"Final model target scaler mean: {target_scaler.mean_[0]:.4f}, scale: {target_scaler.scale_[0]:.4f}")
    ...
    if feature_selection and feature_importances:
        try:
            ...
            for fold_data in feature_importances:
                # Validate feature-importance alignment
                if len(all_features) != len(fold_data['importances']):
                    raise ValueError(f"Mismatch between number of features ({len(all_features)}) and importances ({len(fold_data['importances'])})")
...
def tune_hyperparameters(outer_train_data, outer_train_additive, param_grid, 
    ...
    if outer_train_data.isna().any().any() or outer_train_additive.isna().any().any():
        raise ValueError("Training data for hyperparameter tuning contains missing values")
        ...
        for inner_fold, (inner_train_idx, inner_val_idx) in enumerate(inner_kf.split(training_additive_raw), 1):
            ...
            if feature_selection:
                selector = SelectFromModel(
                    ...
                    threshold=params['feature_threshold']
                )
            ...
            # Print scaler parameters
            print(f"Inner fold {inner_fold} target scaler mean: {target_scaler.mean_[0]:.4f}, scale: {target_scaler.scale_[0]:.4f}")
            
            # Shuffle training data
            X_inner_train_selected, y_inner_train_scaled = shuffle(
                X_inner_train_selected, y_inner_train_scaled, random_state=RANDOM_STATE + inner_fold
            )
    ...
    best_threshold = best_params['feature_threshold']
...
def run_cross_validation(training_file, training_additive_file, testing_file=None, testing_additive_file=None, 
    ...
    if has_test_data:
        ...
        if testing_data.isna().any().any() or testing_additive.isna().any().any():
    ...
    param_grid = {
        ...
        'epochs': [5, 10, 20],
        'feature_threshold': ['0.5*median', 'median', '1.25*median', '1.5*median']
    }
    ...
    for outer_fold, (outer_train_idx, outer_val_idx) in enumerate(outer_kf.split(training_additive_raw), 1):
        ...
        # Log fold details
        logger.info(f"Fold {outer_fold}: Training samples={len(outer_train_idx)}, Validation samples={len(outer_val_idx)}")
        ...
        if feature_selection:
            ...
            logger.info(f"Fold {outer_fold}: Training features={X_outer_train_selected.shape[1]}, Validation features={X_outer_val_selected.shape[1]}")
        ...
        # Shuffle training data
        X_outer_train_selected, y_outer_train_scaled = shuffle(
            X_outer_train_selected, y_outer_train_scaled, random_state=RANDOM_STATE + outer_fold
        )
    ...
    if has_test_data:
        ...
        # Re-tune hyperparameters on full training data
        print("\n=== Tuning Hyperparameters for Final Model ===")
        best_params, best_threshold, _ = tune_hyperparameters(
            outer_train_data=training_data,
            outer_train_additive=training_additive,
            param_grid=param_grid,
            feature_selection=feature_selection,
            inner_n_splits=inner_n_splits
        )
