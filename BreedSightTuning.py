...
def tune_hyperparameters(training_data, training_additive, param_grid, 
                        feature_selection=True, outer_n_splits=2, inner_n_splits=2):
...
def run_cross_validation(training_file, training_additive_file, testing_file, testing_additive_file, 
    ...
    Returns all results including predictions, plots, and downloadable files
    """
    ...
    try:
        # Read files (using .name to get the file path from Gradio's File object)
    ...
    cv_results = KFoldCrossValidation(
        ...
        epochs=1,  # You might want to increase this for real runs
        learning_rate=best_params['learning_rate'],
        batch_size=best_params['batch_size'],
        l2_reg=0.0001,
        dropout_rate=best_params['dropout_rate'],
    ...
    outputs = [
        ...
        cv_results['test_predictions'] if not cv_results['test_predictions'].empty else pd.DataFrame(),
        cv_results['train_plot'],
        cv_results['val_plot'],
        cv_results['test_plot'] if cv_results['test_plot'] else None,
        cv_results['train_csv'],
        cv_results['val_csv'],
        cv_results['test_csv'] if cv_results['test_csv'] else None
    ]

    # Add tuning results to the interface
    tuning_results_df = pd.DataFrame(tuning_results)
    outputs.append(tuning_results_df)
